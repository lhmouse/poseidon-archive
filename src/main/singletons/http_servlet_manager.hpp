...
struct HttpServletManager {
	...
	static boost::shared_ptr<HttpServlet> registerServlet(unsigned port, const std::string &uri,
		const boost::weak_ptr<const void> &dependency, const HttpServletCallback &callback);

	static boost::shared_ptr<const HttpServletCallback> getServlet(unsigned port,
