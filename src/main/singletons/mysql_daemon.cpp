...
namespace MySql {
	namespace {
		...
		private:
			const AsyncSaveCallback m_callback;
			const bool m_succeeded;
			const boost::uint64_t m_autoId;
			const ExceptionCallback m_except;

		public:
			SaveCallbackJob(AsyncSaveCallback callback, bool succeeded, boost::uint64_t autoId,
				ExceptionCallback except)
		...
		private:
			const AsyncLoadCallback m_callback;
			const bool m_found;
			const ExceptionCallback m_except;

		public:
			LoadCallbackJob(AsyncLoadCallback callback, bool found,
				ExceptionCallback except)
		...
		private:
			const BatchAsyncLoadCallback m_callback;
			const std::vector<boost::shared_ptr<ObjectBase> > m_objects;
			const ExceptionCallback m_except;

		public:
			BatchLoadCallbackJob(BatchAsyncLoadCallback callback,
				std::vector<boost::shared_ptr<ObjectBase> > objects,
				ExceptionCallback except)
		...
		public:
			...
			virtual void execute(std::string &query, Connection &conn) = 0;
		};
		...
		private:
			...
			const boost::shared_ptr<const ObjectBase> m_object;
			const bool m_toReplace;

			AsyncSaveCallback m_callback;
			const ExceptionCallback m_except;

		public:
			SaveOperation(boost::uint64_t dueTime,
				boost::shared_ptr<const ObjectBase> object, bool toReplace,
		...
		public:
			...
			void execute(std::string &query, Connection &conn){
				PROFILE_ME;
		...
		private:
			const boost::shared_ptr<ObjectBase> m_object;
		...
		public:
			LoadOperation(boost::shared_ptr<ObjectBase> object, std::string query,
		...
		private:
			...
			boost::shared_ptr<ObjectBase> (*const m_factory)();
			const std::string m_query;

			BatchAsyncLoadCallback m_callback;
			const ExceptionCallback m_except;

		public:
			BatchLoadOperation(const char *tableHint, boost::shared_ptr<ObjectBase> (*factory)(), std::string query,
		...
		public:
			...
			std::size_t getProfile(std::vector<SnapshotItem> &ret, unsigned thread) const {
				const boost::mutex::scoped_lock lock(m_profileMutex);
	...
	boost::shared_ptr<Connection> Daemon::createConnection(){
		return Connection::create(g_serverAddr, g_serverPort,
			g_username, g_password, g_schema, g_useSsl, g_charset);
	}

	std::vector<SnapshotItem> Daemon::snapshot(){
	...
	void Daemon::waitForAllAsyncOperations(){
		for(std::size_t i = 0; i < g_threads.size(); ++i){
			g_threads[i]->waitTillIdle();
		}
	}

	void Daemon::enqueueForSaving(boost::shared_ptr<const ObjectBase> object, bool toReplace,
		AsyncSaveCallback callback, ExceptionCallback except)
	{
	...
	void Daemon::enqueueForLoading(boost::shared_ptr<ObjectBase> object, std::string query,
		AsyncLoadCallback callback, ExceptionCallback except)
	{
		const AUTO(tableName, object->getTableName());
		commitOperation(tableName,
	...
	void Daemon::enqueueForBatchLoading(boost::shared_ptr<ObjectBase> (*factory)(),
