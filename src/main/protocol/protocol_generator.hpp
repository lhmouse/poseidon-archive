...
#define FIELD_ARRAY(name_, mem_)	struct ElementOf ## name_ ## _ {	\
										mem_	\
									};	\
...
#define FIELD_ARRAY(name_, fields_)		const unsigned long long countOf ## name_ ## _ = cur_.name_.size();	\
...
#define FIELD_VINT50(name_)				if(!::Poseidon::vint50FromBinary(cur_.name_, read_, buffer_.size())) THROW_EOS_;
#define FIELD_VUINT50(name_)			if(!::Poseidon::vuint50FromBinary(cur_.name_, read_, buffer_.size())) THROW_EOS_;
#define FIELD_STRING(name_)				unsigned long long countOf ## name_ ## _;	\
										if(!::Poseidon::vuint50FromBinary(countOf ## name_ ## _, read_, buffer_.size())) THROW_EOS_;	\
										if(buffer_.size() < countOf ## name_ ## _) THROW_EOS_;	\
										cur_.name_.resize(countOf ## name_ ## _);	\
										for(unsigned long long i = 0; i < countOf ## name_ ## _; ++i)	\
											cur_.name_[i] = buffer_.get();
#define FIELD_ARRAY(name_, fields_)		unsigned long long countOf ## name_ ## _;	\
										if(!::Poseidon::vuint50FromBinary(countOf ## name_ ## _, read_, buffer_.size())) THROW_EOS_;  \
										cur_.name_.clear();	\
										for(unsigned long long i = 0; i < countOf ## name_ ## _; ++i){	\
											typedef Cur_::ElementOf ## name_ ## _ Element_;	\
