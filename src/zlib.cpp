...
namespace {
	StreamBuffer real_deflate(StreamBuffer &src, int level, bool gzip){
		StreamBuffer dst;
		::z_stream zs = { };
		try {
			int err_code = ::deflateInit2(&zs, level, Z_DEFLATED, 15 + gzip * 16, 8, Z_DEFAULT_STRATEGY);
			if(err_code != Z_OK){
	...
	StreamBuffer real_inflate(StreamBuffer &src, bool gzip){
		StreamBuffer dst;
		::z_stream zs = { };
		try {
			int err_code = ::inflateInit2(&zs, 15 + gzip * 16);
			if(err_code != Z_OK){
...
StreamBuffer deflate(StreamBuffer &src, int level){
	PROFILE_ME;

	return real_deflate(src, level, false);
}
StreamBuffer inflate(StreamBuffer &src){
	PROFILE_ME;

	return real_inflate(src, false);
}

StreamBuffer gzip_deflate(StreamBuffer &src, int level){
	PROFILE_ME;

	return real_deflate(src, level, true);
}
StreamBuffer gzip_inflate(StreamBuffer &src){
	PROFILE_ME;

	return real_inflate(src, true);
}
