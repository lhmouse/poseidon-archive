...
bool UdpClientBase::has_been_shutdown_read() const NOEXCEPT {
	return SocketBase::has_been_shutdown_read();
}
bool UdpClientBase::has_been_shutdown_write() const NOEXCEPT {
	return SocketBase::has_been_shutdown_write();
}
bool UdpClientBase::shutdown_read() NOEXCEPT {
	return SocketBase::shutdown_read();
}
bool UdpClientBase::shutdown_write() NOEXCEPT {
	return SocketBase::shutdown_write();
}
void UdpClientBase::force_shutdown() NOEXCEPT {
	SocketBase::force_shutdown();
}

int UdpClientBase::poll_read_and_process(unsigned char *hint_buffer, std::size_t hint_capacity, bool readable){
	PROFILE_ME;
...
int UdpClientBase::poll_write(Mutex::UniqueLock &write_lock, unsigned char *hint_buffer, std::size_t hint_capacity, bool writeable){
	PROFILE_ME;
...
void UdpClientBase::on_message_too_large(const SockAddr &sock_addr, StreamBuffer data){
	(void)sock_addr;
	(void)data;
}

bool UdpClientBase::send(const SockAddr &sock_addr, StreamBuffer buffer){
	PROFILE_ME;
