...
Ip_port::Ip_port(const Sock_addr &sock_addr){
	...
	case AF_INET: {
		...
		POSEIDON_THROW_UNLESS(sock_addr.size() >= sizeof(sin), Exception, Rcnts::view("Invalid IPv4 Sock_addr"));
		BOOST_STATIC_ASSERT(sizeof(m_ip) >= INET_ADDRSTRLEN);
		POSEIDON_THROW_UNLESS(::inet_ntop(AF_INET, &(sin.sin_addr), m_ip, sizeof(m_ip)), System_exception);
		m_port = load_be(sin.sin_port);
		break; }
	case AF_INET6: {
		::sockaddr_in6 &sin6 = as_sin6(sock_addr.data());
		POSEIDON_THROW_UNLESS(sock_addr.size() >= sizeof(sin6), Exception, Rcnts::view("Invalid IPv6 Sock_addr"));
		BOOST_STATIC_ASSERT(sizeof(m_ip) >= INET6_ADDRSTRLEN);
		POSEIDON_THROW_UNLESS(::inet_ntop(AF_INET6, &(sin6.sin6_addr), m_ip, sizeof(m_ip)), System_exception);
