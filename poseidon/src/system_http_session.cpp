...
void System_http_session::initialize_once(const Http::Request_headers &request_headers){
	...
	POSEIDON_LOG_INFO("System_http_session authentication succeeded: remote = ", get_remote_info(), ", user = ", user, ", URI = ", request_headers.uri, ", headers = ", request_headers.headers);
