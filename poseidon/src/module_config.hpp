...
POSEIDON_DEFINE_MODULE_CONFIG(path) 在模块被加载时解析指定的配置文件。
  这个宏必须和对应的 POSEIDON_DECLARE_MODULE_CONFIG 在同一个 namespace 中展开。

\*===========================================================================*/

#define POSEIDON_DECLARE_MODULE_CONFIG(prefix_)	\
	/* Declare an access function for this module. This function is going to be defined in `POSEIDON_DEFINE_MODULE_CONFIG`. */	\
	extern ::boost::shared_ptr<const ::Poseidon::Config_file> module_config_require_nifty_();	\
	/* Define getters with the suffix `_raw`. */	\
	inline bool POSEIDON_CAT2(prefix_, _raw) (::std::string &val_, const char *key_){	\
		return module_config_require_nifty_()->get_raw(val_, key_);	\
	}	\
	inline const ::std::string & POSEIDON_CAT2(prefix_, _raw) (const char *key_){	\
		return module_config_require_nifty_()->get_raw(key_);	\
	}	\
	/* Define getters with the suffix `_all_raw`. */	\
	inline ::std::size_t POSEIDON_CAT2(prefix_, _all_raw) (::boost::container::vector< ::std::string> &vals_, const char *key_, bool including_empty_ = false){	\
		return module_config_require_nifty_()->get_all_raw(vals_, key_, including_empty_);	\
	}	\
	inline ::boost::container::vector< ::std::string> POSEIDON_CAT2(prefix_, _all_raw) (const char *key_, bool including_empty_ = false){	\
	...
	inline bool POSEIDON_CAT2(prefix_, ) (T_ &val_, const char *key_){	\
		return module_config_require_nifty_()->get<T_>(val_, key_);	\
	}	\
	template<typename T_>	\
	inline T_ POSEIDON_CAT2(prefix_, ) (const char *key_){	\
	...
	inline bool POSEIDON_CAT2(prefix_, ) (T_ &val_, const char *key_, const Def_val_t_ &def_val_){	\
		return module_config_require_nifty_()->get<T_>(val_, key_, def_val_);	\
	}	\
	template<typename T_, typename Def_val_t_>	\
	inline T_ POSEIDON_CAT2(prefix_, ) (const char *key_, const Def_val_t_ &def_val_){	\
		return module_config_require_nifty_()->get<T_>(key_, def_val_);	\
	}	\
	/* Define getters with the suffix `_all`. */	\
	template<typename T_>	\
	inline ::std::size_t POSEIDON_CAT2(prefix_, _all) (::boost::container::vector<T_> &vals_, const char *key_, bool including_empty_ = false){	\
		return module_config_require_nifty_()->get_all<T_>(vals_, key_, including_empty_);	\
	}	\
	template<typename T_>	\
	inline ::boost::container::vector<T_> POSEIDON_CAT2(prefix_, _all) (const char *key_, bool including_empty_ = false){	\
		return module_config_require_nifty_()->get_all<T_>(key_, including_empty_);	\
	}	\
	//
#define POSEIDON_DEFINE_MODULE_CONFIG(path_)	\
	...
	POSEIDON_MODULE_RAII_PRIORITY(handles_, LONG_MIN){	\
	...
	::boost::shared_ptr<const ::Poseidon::Config_file> module_config_require_nifty_(){	\
		...
		POSEIDON_THROW_UNLESS(module_config_, ::Poseidon::Exception, ::Poseidon::Rcnts::view("The configuration file for this module has not been loaded"));	\
