	...
	Poseidon::enqueue_async_job([]{
		auto promise = Poseidon::MongoDbDaemon::enqueue_for_batch_loading(
			[=](const boost::shared_ptr<Poseidon::MongoDb::Connection> &conn){
				...
				obj->fetch(conn);
				LOG_POSEIDON_FATAL("Loaded: _id = ", obj->get_oid(), ", boolean = ", obj->get_m_boolean(),
					", signed = ", obj->get_m_signed(), ", unsigned = ", obj->get_m_unsigned(), ", double = ", obj->get_m_double(),
					", string = ", obj->get_m_string(), ", datetime = ", obj->get_m_datetime(), ", uuid = ", obj->get_m_uuid(),
					", blob = ", obj->get_m_blob(), ", regex = ", obj->get_m_regex());
			}, "Foo", { }, 0, 10);
		Poseidon::JobDispatcher::yield(promise, true);

		auto foo = boost::make_shared<Foo>(true, -123, 456, 78.9, "hello world!", UINT64_MAX,
			Poseidon::Uuid("986CDBFA-E18E-F8C6-A1D2-4CB13B1100D5"), "binary", "regex");
